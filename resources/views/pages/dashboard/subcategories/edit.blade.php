...
<div class="p-4 sm:ml-64">
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-4 bg-white">
        <form class="space-y-6" action="/api/subcat/update/{{$subcat->id}}" method="POST">
            @method('PATCH')
            <div>
                <h1 class="py-4 text-lg font-bold">Edit sub-category</h1>

                <label for="name" class="block text-sm font-medium text-gray-700">Name</label>
                <input type="text" id="name" name="name" value="{{$subcat->name}}"
            ...
            <div>
                ...
                <select id="category_id" name="category_id"
                    class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 text-sm">
                    <option value="" disabled {{$subcat->category_id ? '' : 'selected'}}>Select the appropriate category
                        id</option>
