    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-4 bg-white">
        <div
            ...
            <div class="relative">
                ...
                <input type="text" id="table-search-classes"
            ...
            <button type="button"
                class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-blue-600 dark:hover:bg-blue-700 focus:outline-none dark:focus:ring-blue-800"><a
                    href="{{ route('createClass') }}">+
                    Add New School Class</a></button>
        </div>

        <table class="w-full text-sm text-left rtl:text-right text-gray-500" id="class-table">
            ...
            <tbody>
                ...
                <tr class="bg-white border-b border-gray-200 hover:bg-gray-50">
                    ...
                    <th scope="row" class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap">
                        <div class="text-base font-semibold">{{ $class->name }}</div>
                    </th>
                    <td class="px-6 py-4 align-middle">
                        <div class="flex items-center">
                            {{ $class->class_code ?? 'null' }}
                        </div>
                    </td>
                    <td class="px-6 py-4 flex flex-col space-y-1">

                        <form action="/api/classes/delete/{{ $class->class_code }}" method="post"
                            onsubmit="return confirm('Are you sure you want to delete {{ $class->name }}?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
...
<script>
    document.getElementById("table-search-classes").addEventListener("keyup", function () {
